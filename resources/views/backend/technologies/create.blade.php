...
@section('subtitle', 'Nueva Tecnología')
@section('content_header_title', 'Tecnologías')
@section('content_header_subtitle', 'Crear nueva tecnología')
...
<div class="container-fluid">
    ...
    <div class="row justify-content-center">
        <div class="col-md-9 col-lg-8">
            <x-adminlte-card title="Nueva Tecnología"
                             theme="success" icon="fas fa-code">

                <form method="POST"
                      action="{{ route('backend.technologies.store') }}"
                    ...
                    <div class="form-group">
                            ...
                            <select name="technologie_category_id"
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}"
                                            data-color="{{ $category->color_with_fallback }}"
                                            data-icon="{{ $category->icon_with_fallback }}"
                                            {{ old('technologie_category_id') == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <small class="form-text text-muted">
                            ...
                            <a href="{{ route('backend.technology-categories.create') }}" target="_blank">
                                Crear categoría
                            </a>
                        </small>
                    </div>
                    ...
                    <small class="form-text text-muted mb-3">
                        Busca el slug en
                        <a href="https://simpleicons.org" target="_blank">simpleicons.org</a>.
                        Se genera automáticamente desde el nombre.
                        Ej: <code>laravel</code>, <code>vuedotjs</code>, <code>postgresql</code>
                    </small>
                    ...
                    <div class="form-group">
                        ...
                        @enderror
                        <small class="form-text text-muted">
                            Formato: #RRGGBB — Ej: #FF2D20 (Laravel), #3776AB (Python)
                        </small>
                    </div>
                                ...
                                <select name="proficiency_level"
                                    ...
                                    @foreach([
                                        ...
                                        <option value="{{ $value }}"
                                                ...
                                                {{ old('proficiency_level', 'intermedio') === $value ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="proficiency_percentage">
                                    ...
                                    <span id="percentage-value" class="badge badge-primary ml-1">50%</span>
                                </label>
                                <input type="range"
                                       ...
                                       value="{{ old('proficiency_percentage', 50) }}">
                    ...
                    <div class="alert alert-light border text-center mt-3">
                        ...
                        <div id="iconPreviewWrap" style="height:60px; display:flex; align-items:center; justify-content:center;">
                            <span id="iconPlaceholder"
                                  class="badge badge-secondary"
                                  style="font-size:1.5rem; padding:.5rem 1rem;">??</span>
                        </div>
                        <p class="mt-2 mb-0 font-weight-bold" id="namePreview">Nombre de la Tecnología</p>
                        <small class="text-muted" id="categoryPreview">Sin categoría</small>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ============================================
    // PREVIEW SIMPLE ICONS DESDE SLUG
    // ============================================
    slugInput?.addEventListener('input', function () {
        updateSimpleIconsPreview(this.value.trim());
    });

    function updateSimpleIconsPreview(slug) {
        if (!slug) return;
        const color = colorInput.value.replace('#', '') || 'd4af37';
        ...
        img.onerror = function () {
            // Si falla, mostrar placeholder de iniciales
            iconWrap.innerHTML = `<span id="iconPlaceholder"
                class="badge badge-secondary"
    ...
    percentBadge.textContent = percentRange.value + '%';
